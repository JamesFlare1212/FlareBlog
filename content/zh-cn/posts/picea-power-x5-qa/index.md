...
modified: 2024-12-29T22:40:01+08:00
...
> 这是正常现象，系统显示的电量是准确的。通常情况下，电量会稳定在96%左右，之后下降速度会减慢。关于这个问题，我将从技术角度解释其原理。
>
> 任何电池在充满电时，其截止电压为4.2伏。然而，放置一段时间后，电压会自然下降到4.15至4.16伏，这是电池在4.2伏电压附近的特性（化学相关）。这种电压下降会导致电池管理系统（BMS）检测到电压变化，从而相应地调整显示的电量百分比。
>
...
> 感谢您对我们产品的关注和反馈。关于您提到的纹波问题，我们理解您对产品性能的重视。在使用示波器进行测试时，我们设置了100mV的垂直标尺和100us的水平标尺，并采用交流耦合模式，未限制带宽。测试结果如下：
>
> | 电压 (V) | 电流 (A) | X5 纹波 (mV) | C140 纹波 (mV) |
> |---------:|---------:|-------------:|-------------:|
> | 28       | 5        | 235          | **220**      |
> | 28       | 3        | **160**      | 240          |
> | 28       | 1        | **94**       | 210          |
...
> | 5        | 1        | **39**       | 75           |
>
> 我们选择C140作为对比基准，是因为它在业界被广泛认可为一款优秀的产品，具有强大的性能。我们始终致力于在产品的体积、重量和性能之间寻求最佳平衡，以确保用户获得最佳的使用体验。
>
> 我们理解您对电源输出质量的关注，但我们相信，通过综合考虑各方面因素，我们的产品能够满足大多数用户的需求。我们鼓励用户理性看待纹波数据，并基于实际使用体验来评估产品的整体性能。
...
### 系统菜单的各个选项都代表了什么，有什么用呢？

> - `DIS THD` - Discharge Threshold：放电的截止值，电量放到这个百分比就会停止放电。
> - `CHG THD` - Charge Threshold：充电的截止值，电量充到这个百分比就会停止充电。
> - `CHG POW` - Charge Power：充电的功率，可以设置充电的功率。
> - `AUT OFF` - Auto Off：{{< version "6015" new "./#" X5固件>}} 自动关机，可以设置自动关机的时间。设置一个较长的时间可以避免在小电流设备充电时电源关机。
> - `TFT DIR` - TFT Direction：屏幕的显示方向，可以设置屏幕的显示方向。
> - `C2 PPS` - C2 PPS：{{< version "6021" new "./#" X5固件>}} 可以设置PPS协议支持的最高电流，在 {{< version "6022" changed "./#" X5固件>}} 版本中将精度提升至小数点后两位。有些设备在PPS播报的时候比较死板，只比较3.25。
> - `AUT DC` - Auto DC：{{< version "6025" new "./#" X5固件>}} 可以设置是否自动开启XT-30接口，在 {{< version "6035" changed "./#" X5固件>}} 版本中改善了自动开关的响应速度。
> - `AUT BL` - Auto Backlight：{{< version "6040" new "./#" X5固件>}} 可以设置是否自动调节背光，屏幕亮度是否随环境自动调整。
> - `C2 DPM` - Auto DP / DM：{{< version "6040" new "./#" X5固件>}} 关闭后，用DP和DM的所有快充协议就关闭了。比如SCP就会关闭，此时就只响应PD的快充协议。
