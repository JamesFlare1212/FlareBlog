---
title: "修复在 RTX 4090 上使用 SGLang 运行 GLM-4.7-Flash 时的 OutOfResources: shared memory 错误"
subtitle:
...
早些时候，我在这篇文章里提到了在 SGLang 运行 Qwen3-30B-A3B-FP8 时遇到的 OutOfResources: shared memory 问题。

{{< link "../tuning-fused-moe-triton" "修复在 RTX 4090 上使用 SGLang 运行 Qwen3 MoE 时的 OutOfResources: shared memory 错误" "source of Qwen3 Fused MoE Triton" true >}}

当时我使用了 `tuning_fused_moe_triton.py` 这个脚本来调优 Fused MoE Triton 配置。

不过截止 v0.5.8 版本，

1. 这个脚本不支持 `Glm4MoeLiteForCausalLM` 架构。它没法获取 GLM-4.7-Flash 系列模型正确的 E/N 参数。可能的原因是直到 Transformer 5.x 才支持 GLM-4.7-Flash，而现在 SGLang 所用的版本还是 4.57.1。
2. 另外就是这个脚本没有对 `OutOfResources: shared memory` 做额外的处理，比如跳过过大的 `num_stages`。虽然不会有什么错误，但是会花费大量时间在尝试不可能的组合上。在我的单张RTX 4090 48G的环境下，微调一次要几乎10小时。

## 如何修复？

首先添加 `Glm4MoeLiteForCausalLM` 支持到 `common_utils.py`。然后通过为每个（BLOCK_M,N,K,GROUP_M,warps）缓存“共享内存失败阈值”，并跳过相同tile shape下更大或相等的 `num_stages` 来加速调优（实测有4-5倍提速）。
...
### 修改版 tuning_fused_moe_triton.py
...
### 修改版 tuning_fused_moe_triton_sep.py
...
这里我默认你安装了Transformer 5.x，参考 [GLM-4.7-Flash Model Card](https://huggingface.co/zai-org/GLM-4.7-Flash#sglang)。

```bash
pip install git+https://github.com/huggingface/transformers.git@76732b4e7120808ff989edbd16401f61fa6a0afa
```

详细细节我就不多赘述了，可以参考先前的文章。
...
`tuning_fused_moe_triton_sep.py` 我还没测试，如果有人测试了可以分享一下。
